...
- **`Makefile.common`**: Common Makefile variables and functions.
- **`lib.Makefile`**: Library of Makefile utilities.
...
| :--- | :--- | :--- |
| **`calico/base`** | Base image for other toolchains. | GNU C/C++ libraries, licenses, global configs. (UBI 8, 9, & 10 variants). |
| **`calico/binfmt`** | Enables multi-arch builds on Linux. | QEMU static binaries, registration tool for `binfmt_misc`. Versions defined in `versions.yaml`. |
...
This repository includes specialized Gemini skills located in `.gemini/skills/` to automate common maintenance tasks:

- **`update-toolchain-images`**: A dispatcher skill that identifies which toolchain image to update based on user request.
- **`update-go-build`**: Automates updating Go and Kubernetes versions in `images/calico-go-build/versions.yaml`.
- **`update-rust-build`**: Automates updating the Rust version in `images/calico-rust-build/versions.yaml`.
- **`update-binfmt`**: Automates updating the QEMU version in `images/calico-binfmt/versions.yaml`.