...
(`../test/fixture`) is built this way in CI as a regression guard. Note the
fixture uses `-gc=precise`, so it links go-re2's `libc++.a` but not nottinygc's
`libmimalloc.a` (that archive is only pulled in under `-gc=custom`, as the real
WAF build uses). Both archives reference the same ctype/wctype family, so the
